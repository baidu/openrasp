...
IncludeObject::IncludeObject(zval *filename, const std::string &document_root, const std::string &function, bool plugin_filter)
...
{
    if (function.empty() || realpath.empty())
    {
        return false;
    }
    if (plugin_filter)
    {
        const char *scheme_pos = determine_scheme_pos(Z_STRVAL_P(filename));
        if (nullptr == scheme_pos)
        {
            std::string param = std::string(Z_STRVAL_P(filename), Z_STRLEN_P(filename));
            ...
            {
                if (param.find("../") == std::string::npos && (!document_root.empty() && openrasp::start_with(realpath, document_root)))
                {
                    return false;
                }
            }
