...
public class JsPluginManager {
                ...
                new FileScanListener() {
                    ...
                    public void onFileCreate(File file) {
                        if (file.getName().endsWith(".js")) {
                            updatePluginAsync(null, null, null);
                        }
                    }
    ...
    private synchronized static void updatePlugin(String plugin, String md5, String version) throws Exception {
        // 清空 algorithm.config 配置
        Config.getConfig().setAlgorithmConfig("{}");
        boolean oldValue = HookHandler.enableHook.getAndSet(false);
        JSContextFactory.setCloudCheckScript(plugin, md5, version);
        HookHandler.enableHook.set(oldValue);
    }
            ...
            public void run() {
                try {
                    if (plugin != null && md5 != null && version != null) {
                        updatePlugin(plugin, md5, version);
                    } else {
                        updatePlugin();
                    }
                } catch (Exception e) {
