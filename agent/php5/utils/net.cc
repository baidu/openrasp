...
void fetch_if_addrs(std::map<std::string, std::string> &if_addr_map)
{
    ...
    {
        openrasp_error(LEVEL_WARNING, LOG_ERROR, _("Error allocating memory needed to call GetAdaptersinfo."));
    }
    ...
    {
        openrasp_error(LEVEL_WARNING, LOG_ERROR, _("getifaddrs() error: %s"), strerror(errno));
    }
    else
    {
        int n, s;
        ...
        for (ifa = ifaddr, n = 0; ifa != NULL; ifa = ifa->ifa_next, n++)
        {
            ...
            if (ifa->ifa_addr->sa_family == AF_INET)
            {
                ...
                if (s != 0)
                {
                    openrasp_error(LEVEL_WARNING, LOG_ERROR, _("getifaddrs() error: getnameinfo failed - %s."), gai_strerror(s));
                }
...
bool fetch_source_in_ip_packets(char *local_ip, size_t len, char *url)
{
    ...
    if (nullptr == p)
    {
        openrasp_error(LEVEL_WARNING, LOG_ERROR, _("inet_ntop() error: %s"), strerror(errno));
        close(sock);
        return false;
    }
    close(sock);
