...
PHP_MINIT_FUNCTION(openrasp_fswatch)
{
    ...
    {
        //openrasp_error(LEVEL_ERR, CONFIG_ERROR, _("SAPI %s does not support automatic reloading, file monitor is deactivated"), sapi_module.name);
        return SUCCESS;
    }
#else
    ...
    *(void **)(&ap_signal_parent) = GetProcAddress(module, "ap_signal_parent");
    ...
    try
    {
        ...
        monitor = fsw::monitor_factory::create_monitor(
            ...
            [](const std::vector<fsw::event> &events, void *ctx) {
                try
                {
                    ...
                    {
                        openrasp_error(LEVEL_ERR, CONFIG_ERROR, _("Failed to reload %s"), sapi_module.name);
                    }
#else
                    ap_signal_parent(2);
#endif
                ...
                {
                    openrasp_error(LEVEL_ERR, CONFIG_ERROR, _("An exception occurred while reloading master process, but this message may not be visible"));
                }
            } TSRMLS_CC);
        ...
        fswatch_thread = new std::thread([]() {
            ...
            {
                openrasp_error(LEVEL_ERR, FSWATCH_ERROR, _("Fswatch error: %s"), conf.what());
            }
            catch (...)
            {
                openrasp_error(LEVEL_ERR, FSWATCH_ERROR, _("Fswatch error: unknown error"));
            }
            delete monitor;
            monitor = nullptr;
        });
    ...
    {
        openrasp_error(LEVEL_ERR, FSWATCH_ERROR, _("Failed to initialize fswatch: %s"), conf.what());
    }
    catch (...)
    {
        openrasp_error(LEVEL_ERR, FSWATCH_ERROR, _("Failed to initialize fswatch: unknown error."));
    }
