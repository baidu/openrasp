...
void openrasp_shared_alloc_create_lock(void)
{
	...
	if (!memory_mutex) {
		openrasp_error(LEVEL_ERR, SHM_ERROR, _("Cannot create mutex"));
		return;
	}
...
void openrasp_shared_alloc_lock_win32(void)
{
	...
	if (waitRes == WAIT_FAILED) {
		openrasp_error(LEVEL_ERR, SHM_ERROR, _("Cannot lock mutex"));
	}
}
...
static int openrasp_shared_alloc_reattach(openrasp_shared_segment_globals **shared_segments_p, char **error_in)
{
	...
	err = GetLastError();
	if (!fp) {
		openrasp_error(LEVEL_ERR, SHM_ERROR, _("Unable to open base address file"));
		*error_in="fopen";
		return ALLOC_FAILURE;
	}
	if (!fscanf(fp, "%p", &wanted_mapping_base)) {
		err = GetLastError();
		openrasp_error(LEVEL_ERR, SHM_ERROR, _("Unable to read base address"));
	...
	if (VirtualQuery(wanted_mapping_base, &info, sizeof(info)) == 0 ||
		...
		openrasp_error(LEVEL_ERR, SHM_ERROR, _("Unable to reattach to base address, requested address space isn't free"));
		return ALLOC_FAILURE;
   	}
	...
	if (mapping_base == NULL) {
		openrasp_error(LEVEL_ERR, SHM_ERROR, _("Unable to reattach to base address: %d"), err);
...
static int create_segments(openrasp_shared_segment_globals **shared_segments_p, char **error_in)
{
	...
	if (map_retries == MAX_MAP_RETRIES) {
		openrasp_shared_alloc_unlock_win32();
		openrasp_error(LEVEL_ERR, SHM_ERROR, _("Unable to open file mapping"));
	...
	*shared_segments_p = (openrasp_shared_segment_globals *) calloc(1, requested_size);
	if (!*shared_segments_p) {
		openrasp_shared_alloc_unlock_win32();
		openrasp_error(LEVEL_ERR, SHM_ERROR, _("calloc() failed"));
		*error_in = "calloc";
		return ALLOC_FAILURE;
	}
	...
	if (memfile == NULL) {
		openrasp_shared_alloc_unlock_win32();
		openrasp_error(LEVEL_ERR, SHM_ERROR, _("Unable to create file mapping"));
	...
	if (mapping_base == NULL) {
		openrasp_shared_alloc_unlock_win32();
		openrasp_error(LEVEL_ERR, SHM_ERROR, _("Unable to create view for file mapping"));
		*error_in = "MapViewOfFile";
		...
		if (!fp) {
			openrasp_shared_alloc_unlock_win32();
			openrasp_error(LEVEL_ERR, SHM_ERROR, _("Unable to write base address"));
			return ALLOC_FAILURE;
		}
		fprintf(fp, "%p\n", mapping_base);
		fclose(fp);
