...
{
    TSRMLS_FETCH();
    char *body = fetch_request_body(body_len TSRMLS_CC);
    if (body)
    {
...
void Parameter::update_form_str()
{
    ...
    if (IS_ARRAY == Z_TYPE_P(http_post) && zend_hash_num_elements(Z_ARRVAL_P(http_post)) > 0)
    {
...
void Parameter::update_multipart_files()
{
    ...
    if (IS_ARRAY == Z_TYPE_P(http_files) && zend_hash_num_elements(Z_ARRVAL_P(http_files)) > 0)
    {
             ...
             zend_hash_move_forward(ht))
        {
                ...
                Z_TYPE_PP(name_value) == Z_TYPE_PP(tmp_name_value))
            {
                ...
                {
                    recursive_restore_files({std::string(key)}, *name_value, *tmp_name_value);
                }
                else if (type == HASH_KEY_IS_LONG)
                {
                    long actual = idx;
                    recursive_restore_files({std::to_string(actual)}, *name_value, *tmp_name_value);
                }
            }
        }
    }
