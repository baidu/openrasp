          ...
          <div class="page-options d-flex" style="margin-top: 5px;">
            <b-dropdown text="应用语言" class="ml-4" right>
              <b-container style="width: 250px;">
                <b-form-row>
                  <template v-for="(key, index) in language_list">
                    <b-col :key="key">
                      <label class="custom-switch">
                        <input v-model="selected_language" type="checkbox" class="custom-switch-input" :value="key">
            ...
            <div class="input-icon ml-2">
              ...
              <input v-model.trim="hostname" type="text" class="form-control w-10" placeholder="主机名称" @keyup.enter="loadEvents(1)">
            </div>
          </div>
        </div>
        <div class="page-options d-flex" style="margin-top: 5px;">
          <div class="input-icon ml-2">
            ...
            <input v-model.trim="crash_message" type="text" class="form-control w-10" placeholder="报警信息" style="width: 210px" @keyup.enter="loadEvents(1)">
          </div>
          <div class="input-icon ml-2">
              <span class="input-icon-addon">
                <i class="fe fe-search" />
              </span>
            <input v-model.trim="rasp_id" type="text" class="form-control w-10" placeholder="主机ID" @keyup.enter="loadEvents(1)">
          </div>
        </div>
        <div class="page-options d-flex" style="margin-top: 5px;">
...
export default {
  ...
  watch: {
    ...
    selected_language() { this.loadEvents(1) }
  },
  mounted() {
  ...
  methods: {
    ...
    loadEvents(page) {
      this.loading = true
      return this.request.post('v1/api/log/crash/search', {
        data: {
